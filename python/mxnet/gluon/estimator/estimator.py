...
class Estimator(object):
    ...
    def __init__(self, net, trainers,
                 loss=None,
                 metrics=None,
                 ctx=None):

        self.net = net
        # TODO: What if loss is None
        if isinstance(loss, gluon.loss.Loss):
            self.loss = [loss]
        else:
            self.loss = loss or []
        if not self.loss:
            warnings.warn("No Loss specified, default softmax cross-entropy loss is used.")
            self.loss = [gloss.SoftmaxCrossEntropyLoss()]

        # TODO: What if metric is None
        if isinstance(metrics, EvalMetric):
            self.metrics = [metrics]
        else:
            self.metrics = metrics or []
        if not self.metrics:
            warnings.warn("No metrics specified, default accuracy metric is used.")
        ...
        # TODO: Is there any way to check if net is already initialize??
        ...
        if isinstance(ctx, Context):
            self.ctx = [ctx]
        if isinstance(ctx, list) and len(ctx) > 0 and isinstance(ctx[0], Context):
            self.ctx = ctx
        if not ctx:
    ...
    def fit(self, train_data,
        ...
        for epoch in range(epochs):
            ...
            for handler in event_handlers:
                if isinstance(handler, LoggingHandler) and verbose == 0:
                    continue
                handler.epoch_begin()
            ...
            for i, batch in enumerate(train_data):
                ...
                # batch begin
                for handler in event_handlers:
                    if isinstance(handler, LoggingHandler) and verbose < 2:
                        continue
                    handler.batch_begin()
